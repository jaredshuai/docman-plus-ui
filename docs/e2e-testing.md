...
- 默认后端网关：`http://localhost/prod-api`
- 如本机联调环境不在默认地址，使用环境变量 `DOCMAN_E2E_BASE_URL` 覆盖
3. **测试账号**:
   - 用户名：`admin`
   - 密码：`admin123`
   - 验证码已关闭
...
|------|------|
| 登录成功后跳转到首页 | 验证登录流程正常 |
| 仪表盘页面正常加载 | 验证领导概览四个摘要卡片可见 |
| 仪表盘区域内容可见 | 验证项目级概览、重点关注项目、超期节点、文档完善度区域 |
| 项目管理页面可打开 | 验证搜索表单和列表区域 |
| 项目页可进入领导概览 | 验证项目主页入口可达 |
| 文档中心页面可打开 | 验证文档表格存在 |
...
1. 检查后端服务是否启动
2. 检查默认地址 `http://localhost` 或自定义 `DOCMAN_E2E_BASE_URL` 是否可访问
3. 检查测试账号是否正确
